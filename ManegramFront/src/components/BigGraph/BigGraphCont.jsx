...
function BigGraphCont(){
    ...
    useEffect(()=>{
        ...
        axios.get(`http://127.0.0.1:8000/container-log/${selected.container_id}&${startDate}&${endDate}` ,{headers: {'Authorization': `bearer ${localStorage.getItem('accsess_token')}`} } ).then((response)=>{
    ...
    function OpenMessages(get){
        axios.get(`http://127.0.0.1:8000/container-log/${selected.container_id}&${startDate}&${endDate}/${get}` ,{headers: {'Authorization': `bearer ${localStorage.getItem('accsess_token')}`} } ).then((response)=>{
    ...
    return <div className='d-flex flex-column align-items-center justify-content-center' >
        ...
        <div  style={{height:'300px' , width:'80%'}} className='m-5 d-flex d-column justify-content-center align-items-center'>
          ...
          <ResponsiveContainer width='100%' height='100%'>
              ...
              <Tooltip />
              <Legend/>
                  <Bar  dataKey='sent_message_count' stroke='#053f96' fill='#053f96' />
                  <Bar  dataKey='added_message_count' stroke='#258f31' fill='#258f31' />
                  <Bar  dataKey='deleted_message_count' stroke='#8a0f13' fill='#8a0f13' />
              </BarChart>
          </ResponsiveContainer>
          ...
          <div className='w-50 p-4 rounded-3' style={{backgroundColor:'#081D3C'}}>
              ...
              <div className='d-flex text-center btnBluee justify-content-between mb-4 row'>
                  <div  className=' col-12 col-md m-2'>
                    ...
                    <button onClick={()=> OpenMessages('sent-messages')} className=' btnBluee'>See Sent</button>
                  </div>

                  <div  className=' col-12 col-md m-2'>
                    <h2>{counts['added_in_period']}</h2>
                    <button onClick={()=> OpenMessages('added-messages')}  className=' btnBluee'>See Added</button>
                  </div>

                  <div  className=' col-12 col-md m-2'>
                    <h2>{counts['deleted_in_period']}</h2>
                    <button onClick={()=> OpenMessages('deleted-messages')}  className=' btnBluee'>See Deleted</button>
                  </div>
